...
// ---------------------------------------------------------------------------
// Bid Author: role logins, per-role response sections, forms, assigned RFPs
// ---------------------------------------------------------------------------

/** The specialist roles that log in to Bid Author (everyone except Bid Manager). */
export const AUTHOR_ROLES: RoleId[] = ROLE_ORDER.filter(
  (r) => r !== "bid-manager",
);

/** Proposal sections this role is responsible for authoring. */
export function sectionsForRole(roleId: RoleId): Section[] {
  return SECTIONS.filter((s) => s.contributor === roleId);
}

/** Annexures / forms this role must fill. */
export function formsForRole(roleId: RoleId): BidForm[] {
  return TEAM_FORMS.filter((f) => f.contributor === roleId);
}

/** The person who logs in for a role. */
export function personForRole(roleId: RoleId): Person {
  return personById(DEFAULT_ASSIGNEE[roleId])!;
}

export type WorkStatus = "Awaiting response" | "In progress" | "Submitted";

export interface AssignedRfp {
  id: string;
  title: string;
  authority: string;
  due: string;
  value: string;
  status: WorkStatus;
  /** Only the primary tender is fully interactive in this prototype. */
  interactive: boolean;
}

const primary = RFPS[0];

export const ASSIGNED_RFPS: AssignedRfp[] = [
  {
    id: primary.id,
    title: primary.title,
...
/** The primary RFP object (full triage data) for the work screen. */
export const PRIMARY_RFP = primary;
